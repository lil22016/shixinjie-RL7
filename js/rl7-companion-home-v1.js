...
var state=null, timer=null, wakeLock=null, bubbleTimer=null;
...
function defaults(){return {mode:'study',focusMinutes:25,breakMinutes:5,phase:'focus',running:false,endAt:0,remaining:1500,task:'',completed:0,keepAwake:false}}
...
function addUI(){
 ...
 d.innerHTML=`<div class="rl7-lc-shell">
   ...
   <div class="rl7-lc-stage" id="rl7-lc-stage">
      ...
      <div class="rl7-lc-placeholder"><i class="fas fa-crown"></i><div>Stay with Loki</div><small>Tap anywhere to interact</small></div>
   </div>
...
function show(){load();addUI();document.getElementById('rl7-loki-companion').classList.add('on');render();if(!timer)timer=setInterval(tick,500);if(state.keepAwake)requestWake();bubble(state.mode==='study'?pick(support):pick(stay))}
...
function openSettings(){document.getElementById('rl7-lc-settings-panel').classList.add('on');document.getElementById('rl7-lc-focus').value=state.focusMinutes;document.getElementById('rl7-lc-break').value=state.breakMinutes;document.getElementById('rl7-lc-awake').checked=!!state.keepAwake}
