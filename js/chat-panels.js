...
/* === 面板高度与移动端输入法对齐 ===
   --chat-panel-h 已由 app.js _syncKbd 统一维护（修复4：消除两套键盘高度驱动各自动画造成的闪动），
   此处仅保留初始兜底值（无 visualViewport 的极旧浏览器也能正确展开面板）。 */

/* 全局点击外部关闭：点击面板 / 输入区之外任意空白处，关闭表情面板与加号菜单（修复3）。
   用 mousedown 而非 click，兼容安卓旧 WebView 并对触屏即时响应。 */
function _initGlobalPanelDismiss() {
  try {
    document.documentElement.style.setProperty('--chat-panel-h', '300px');
  } catch (e) {}
  document.addEventListener('mousedown', function (e) {
    var t = e.target;
    if (!t || !t.closest) return;
    // 点击面板或输入区内部不关闭
    if (t.closest('.chat-panel-area, .sticker-panel, .plus-panel, .chat-input-zone')) return;
    if (_stickerPanelOpen) closeStickerPanel();
    // closePlusMenu 为函数声明（js/chat-extra.js），事件回调执行时已加载为全局函数
    if (typeof closePlusMenu === 'function') closePlusMenu();
  });
}
if (typeof window !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _initGlobalPanelDismiss);
  } else {
    _initGlobalPanelDismiss();
  }
}
...
function invalidateStickerCache() {
  _stickerCache = null;
  // 若聊天表情面板当前已展开，立即重载重渲染，保证表情包管理页的变更即时生效
  if (_stickerPanelOpen) {
    loadStickersForPanel();
  }
}
...
function renderStickerTabs() {
  ...
  // 滚动到激活的 tab（修复4：instant 即时滚动，避免每次渲染 smooth 平滑滚动与键盘高度重排叠加抖动）
  setTimeout(function() {
...
function renderEmojiGrid(gridEl) {
  ...
  // 最近使用（过滤掉已删除的 emoji，再截断为最多9个——兼容历史残留数据）
