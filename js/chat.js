...
var _sheetPanelLock = function (on) {
  var area = document.getElementById('chat-panel-area');
  if (!area) return;
  if (on) area.classList.add('sheet-locked');
  else area.classList.remove('sheet-locked');
};

// 对方主动拍一拍混入概率：普通自动回复链路中对方主动发送拍一拍的触发概率（10%）
// （区别于 PAT_AUTO_REPLY_MIX_PROBABILITY：那是「我发拍一拍后对方回拍」的概率，这里是「普通聊天中对方主动拍一拍」的概率）
var PAT_ACTIVE_MIX_PROBABILITY = 0.1;

/* === 消息分批渲染：解决消息过多时全量渲染卡顿 ===
   DISPLAYED_MSG_COUNT：聊天界面初始渲染的最近消息条数
   HISTORY_BATCH_SIZE：点击/滚动「加载更多历史」时每次追加的条数 */
...
function _msgSearchText(msg) {
  ...
  if (msg.msgType === 'forward') return '[聊天记录] 来自' + ((msg.forwardFrom && msg.forwardFrom.name) || '对方') + '的转发';
  return msg.text || '';
}
...
function bindChatTapMenu(container) {
  ...
  container.addEventListener('click', function(e) {
    ...
    // 转发卡片点击打开转发详情层，不弹操作菜单（多选/删除等走多选模式）
    if (el.classList.contains('forward-bubble')) return;
  ...
  container.addEventListener('dblclick', function(e) {
    // 多选模式：停用双击看大图，避免与勾选冲突
    if (isMultiSelectActive()) { e.preventDefault(); e.stopPropagation(); return; }
...
function showMsgActionMenu(msgId, anchorEl) {
  ...
  var favoriteItem = '<div class="msg-action-item" data-act="favorite" title="收藏"><i class="fas fa-star"></i><span>收藏</span></div>';
  menu.innerHTML =
    ...
    + '<div class="msg-action-item" data-act="multi" title="多选"><i class="fas fa-check-square"></i><span>多选</span></div>'
  ...
  menu.addEventListener('click', function(ev) {
    ...
    else if (act === 'multi') enterMultiSelectMode();
...
function _buildFavorite(msg, chatId) {
  ...
  if (msg.msgType === 'text') { category = 'text'; label = '文本'; text = msg.text || ''; }
  else if (msg.msgType === 'sticker') { category = 'sticker'; label = '表情包'; text = '[表情]'; stickerData = msg.stickerData || ''; }
    ...
    // 语音收藏：保存 base64 音频数据，供收藏页还原播放
    category = 'voice'; label = '语音'; text = '[语音]';
    voiceData = {
    ...
    // 红包收藏：保存完整红包记录（含领取态字段），供收藏页还原展示
    category = 'redpacket'; label = '红包'; text = '[红包]' + (msg.greeting || '恭喜发财');
    rpData = {
      ...
      selfAmount: (msg.selfAmount != null) ? msg.selfAmount : null,
      otherAmount: (msg.otherAmount != null) ? msg.otherAmount : null
    };
  }
    ...
    // 保存完整决策数据（问题、选项、对方勾选结果、已答/待答状态），供收藏页还原展示
    decisionData = {
      ...
      isGroup: !!(msg.decision && msg.decision.isGroup),
      pending: (msg.decision && 'pending' in msg.decision) ? !!msg.decision.pending : !!(msg.decision && msg.decision.pending),
      publishedAt: (msg.decision && msg.decision.publishedAt != null) ? msg.decision.publishedAt : null
    };
  }
...
function renderChatMessages(chatId, opts) {
  ...
  // 分批渲染：只渲染消息数组尾部最近 count 条（按 displayedMessageCount + HISTORY_BATCH_SIZE 控制渲染量），
...
/* 加载更多历史消息：增加渲染条数并重渲染，隐藏容器避免闪烁，保持锚点滚动位置 */
...
function _buildNormalMessageHtml(msg, isSelf, selfAvatarHtml, otherAvatarHtml, suffixHtml, senderName, senderStatusHtml, rowGroupCls) {
  ...
  if (msg.msgType === 'sticker') {
    ...
    // 转发卡片（微信/QQ 式合并转发）：接收方看到一张卡片，点击打开详情层查看原始内容。
    // 形态：标题「<来源>的聊天记录」+ 若干条缩略预览 + 底部灰色「聊天记录」字样 + 右侧方向小三角
    var fItems = msg.forwardItems || [];
    var fFrom = (msg.forwardFrom && msg.forwardFrom.name) ? msg.forwardFrom.name : '对方';
    html += '<div class="message-row ' + (isSelf ? 'self' : 'other' + rowGroupCls) + '" data-msg-id="' + msg.id + '">'
          + (isSelf ? selfAvatarHtml : otherAvatarHtml)
          + '<div class="message-body">'
          + senderHtml
          + '<div class="message-bubble forward-bubble" onclick="openForwardDetail(\'' + msg.id + '\')">'
          + '<div class="forward-card-title"><i class="fas fa-history"></i><span>' + Core.escapeHtml(fFrom) + '的聊天记录</span></div>'
          + '<div class="forward-card-previews">' + _forwardPreviewRowsHtml(fItems, fFrom) + '</div>'
          + '<div class="forward-card-foot"><span>聊天记录</span><i class="fas fa-caret-right"></i></div>'
          + '</div>'
          + suffixHtml
          + '<div class="message-meta">' + _buildReadStatusHtml(msg) + '<div class="message-time">' + Core.formatTime(msg.time) + '</div></div>'
          + '</div>'
...
function sendStickerMessage(stickerData) {
  ...
  // 修复4：发送开始即锁定面板过渡，避免键盘收起+插入大图重排叠加闪动
  _sheetPanelLock(true);

  // 阈值判定压缩：GIF/小表情原样保留，超过阈值才压缩，避免表情包质量损失
...
function sendImageMessage(imageData) {
  ...
  // 阈值判定压缩：GIF/小图原样保留，超过阈值才压缩到 1200px，避免聊天图片质量损失
...
function appendMessage(chatId, msg) {
  ...
  // 表情包/涂鸦/图片类消息常在软键盘弹出/收起过程中下发，此时容器高度还在随键盘布局
  // （--chat-h 单值跟随可视区）变化，滚动需覆盖到键盘稳定之后——统一复用 scrollChatToBottom
  // 的 rAF 多帧 + 80/200/350/500/1100ms 多时点兜底，避免新消息"卡半屏"需手动滑动。
  ChatMedia.resolveDomRefs(container, function() {
    if (container.id === 'chat-messages') scrollChatToBottom();
  });
}
...
function doAutoReply(chatId) {
  ...
  // 对方偶尔把其它聊天里的一条真实消息转发给你（低概率、可配置）。
  // 此分支位于 doAutoReply“对方能正常发送”的条件体内，未开启主动发送就不会被调用，
  // 因此天然满足“在对方能正常发送的前提下才可能触发”。
  if (Storage.getPartnerForwardProb() > 0
      && Math.random() < Storage.getPartnerForwardProb()
      && typeof _partnerForwardTo === 'function') {
    if (_partnerForwardTo(chatId)) return;   // 转发接管本次回复；无来源消息时回落正常回复
  }
...
/* 转发项的单行摘要文本（列表预览、收藏、搜索等场景复用） */
function _forwardItemSummaryText(item) {
  if (!item) return '';
  var t = item.msgType;
  if (t === 'text') return item.text || '';
  if (t === 'sticker') return '[表情]';
  if (t === 'doodle') return '[涂鸦]';
  if (t === 'image') return '[图片]';
  if (t === 'voice') return '[语音] ' + ((item.duration || 0) + '"');
  if (t === 'redpacket') return '[红包] ' + (item.greeting || '');
  if (t === 'gift') return '[礼物] ' + ((item.gift && item.gift.name) || '');
  if (t === 'decision') return '[帮我抉择] ' + ((item.decision && item.decision.question) || '');
  if (t === 'forward') return '[聊天记录]';
  return '';
}

/* 转发卡片的首条预览内容（HTML，支持表情贴纸小图标） */
function _forwardItemPreviewHtml(item) {
  var t = item.msgType;
  if (t === 'sticker' || t === 'doodle' || t === 'image') {
    var ps = ChatMedia.imgSrcFor(item.stickerData || item.imageData || '');
    if (ps && ps.src && !ps.ref) {
      return '<span class="forward-preview-pic"><img src="' + ps.src + '" alt=""></span>';
    }
  }
  return Core.escapeHtml(_forwardItemSummaryText(item));
}

/* 转发卡片的缩略预览行（微信式：每行「来源名: 内容摘要」，最多 4 行，
   第 1 条起截断，其余如图片/表情等用对应摘要文本，底部统一灰色「聊天记录」） */
function _forwardPreviewRowsHtml(items, fromName) {
  if (!items || !items.length) return '';
  var from = fromName || '对方';
  var n = Math.min(items.length, 4);
  var html = '';
  for (var i = 0; i < n; i++) {
    var it = items[i];
    if (!it) continue;
    html += '<div class="forward-card-row"><span class="fcr-name">' + Core.escapeHtml(from)
      + '</span><span class="fcr-text">' + _forwardItemSummaryText(it) + '</span></div>';
  }
  return html;
}

/* 点击转发卡片：打开转发详情层，以各消息的原始形态展示被转发内容 */
function openForwardDetail(msgIdOrMsg) {
  var msg = null;
  var chatId = '';
  if (msgIdOrMsg && typeof msgIdOrMsg === 'object') {
    msg = msgIdOrMsg;
    var roomEl0 = document.getElementById('page-chat-room');
    chatId = roomEl0 ? (roomEl0.dataset.chatId || '') : '';
  } else {
    var roomEl = document.getElementById('page-chat-room');
    chatId = roomEl ? (roomEl.dataset.chatId || '') : '';
    if (!chatId) { Core.toast('无法定位当前聊天'); return; }
    var msgs = Storage.getMessages(chatId);
    for (var i = 0; i < msgs.length; i++) {
      if (String(msgs[i].id) === String(msgIdOrMsg)) { msg = msgs[i]; break; }
    }
  }
  if (!msg || msg.msgType !== 'forward') { return; }
  var items = msg.forwardItems || [];
  var fFrom = (msg.forwardFrom && msg.forwardFrom.name) ? msg.forwardFrom.name : '对方';
  // 渲染上下文：详情内每条转发项需按「各自身份」解析发送者头像/昵称。
  // sourceChatId 记录转发来源聊天，优先按来源解析；旧数据无来源时回退当前聊天上下文。
  var ctx = {
    sourceChatId: (msg.forwardFrom && msg.forwardFrom.chatId) || '',
    currentChatId: chatId,
    currentIsGroup: !!chatId && isGroupChatId(chatId),
    myProfile: Storage.getMyProfile()
  };
  if (!ctx.currentIsGroup && _chatCurrentPartnerId) {
    var pp = Storage.getPartnerProfiles();
    for (var pi2 = 0; pi2 < pp.length; pi2++) {
      if (pp[pi2].id === _chatCurrentPartnerId) { ctx.otherProfile = pp[pi2]; break; }
    }
  }
  if (ctx.currentIsGroup) {
    var gObj = getGroupByChatId(chatId);
    var gMems = gObj ? getGroupMembers(gObj) : [];
    ctx.otherProfileMap = {};
    gMems.forEach(function(m) { ctx.otherProfileMap[m.id] = m; });
  }
  closeForwardDetail();
  var bodyHtml = '';
  items.forEach(function(it, idx) { bodyHtml += _renderForwardDetailItem(it, idx, ctx); });
  var ov = document.createElement('div');
  ov.className = 'forward-target-overlay';
  ov.id = 'forward-detail-overlay';
  ov.innerHTML = '<div class="forward-detail-panel">'
    + '<div class="forward-detail-head"><span class="fwd-head-title"><i class="fas fa-history"></i> 聊天记录</span><i class="fas fa-times forward-target-close"></i></div>'
    + '<div class="forward-detail-sub">来自' + Core.escapeHtml(fFrom) + '的转发 · 共 ' + items.length + ' 条</div>'
    + '<div class="forward-detail-body">' + (bodyHtml || '<div class="forward-detail-empty">没有内容</div>') + '</div>'
    + '</div>';
  document.body.appendChild(ov);
  ov.addEventListener('click', function(e) {
    if (e.target === ov || e.target.closest('.forward-target-close')) closeForwardDetail();
  });
}

/* 解析转发详情条目发送者：返回 { avatarHtml, name }（微信式：按各自身份取头像+昵称） */
function _buildForwardSenderMeta(it, ctx) {
  ctx = ctx || {};
  var myProfile = ctx.myProfile || Storage.getMyProfile();
  var sender = (it && it.sender) || {};
  if (sender.isSelf) {
    return {
      avatarHtml: _buildMessageAvatar(myProfile),
      name: myProfile.nickname || myProfile.name || '我'
    };
  }
  var sourceChatId = ctx.sourceChatId || '';
  var fromId = sender.fromId || '';
  // 1) 群聊来源：按成员 fromId 取真正的发言人
  if (sourceChatId && isGroupChatId(sourceChatId)) {
    var g = getGroupByChatId(sourceChatId);
    var members = g ? getGroupMembers(g) : [];
    for (var i = 0; i < members.length; i++) {
      if (String(members[i].id) === String(fromId)) {
        return { avatarHtml: _buildMessageAvatar(members[i]), name: members[i].nickname || '成员' };
      }
    }
    // 群消息缺 fromId 或成员已不在：用群自身头像/名称兜底
    if (g) {
      var gAvatar = _buildMessageAvatar({ avatar: g.name, nickname: (g.name || '群') + '聊', avatarColor: g.avatarColor || '#7A6FBE', avatarImage: g.avatarImage, avatarShape: g.avatarShape });
      return { avatarHtml: gAvatar, name: g.name || '群聊' };
    }
    return { avatarHtml: '', name: '成员' };
  }
  // 2) 单聊来源：取来源聊天的对方资料（转发项发送者 = 来源聊天的对方）
  if (sourceChatId && sourceChatId.indexOf('partner_') === 0) {
    var pid = sourceChatId.slice('partner_'.length);
    var partners = Storage.getPartnerProfiles() || [];
    for (var j = 0; j < partners.length; j++) {
      if (String(partners[j].id) === pid) {
        return { avatarHtml: _buildMessageAvatar(partners[j]), name: partners[j].nickname || partners[j].name || '对方' };
      }
    }
  }
  // 3) 无来源信息（旧数据/嵌套转发无来源）回退当前聊天上下文
  if (ctx.currentIsGroup) {
    var cm = ctx.otherProfileMap ? (ctx.otherProfileMap[fromId] || null) : null;
    if (cm) return { avatarHtml: _buildMessageAvatar(cm), name: cm.nickname || '成员' };
    if (ctx.otherProfile) return { avatarHtml: _buildMessageAvatar(ctx.otherProfile), name: ctx.otherProfile.nickname || ctx.otherProfile.name || '成员' };
  } else if (ctx.otherProfile) {
    return { avatarHtml: _buildMessageAvatar(ctx.otherProfile), name: ctx.otherProfile.nickname || ctx.otherProfile.name || '对方' };
  }
  return { avatarHtml: '', name: '对方' };
}

function closeForwardDetail() {
  var ov = document.getElementById('forward-detail-overlay');
  if (ov && ov.parentNode) ov.parentNode.removeChild(ov);
}

/* 详情层单条转发项：按原始 msgType 渲染对应形态，顶部带发送者头像+昵称（微信聊天记录样式） */
function _renderForwardDetailItem(it, idx, ctx) {
  if (!it) return '';
  var t = it.msgType || 'text';
  var senderMeta = _buildForwardSenderMeta(it, ctx);
  var nameHtml = Core.escapeHtml(senderMeta.name || '对方');
  var avatarHtml = senderMeta.avatarHtml
    ? '<div class="fwd-item-avatar">' + senderMeta.avatarHtml + '</div>'
    : '<div class="fwd-item-avatar fwd-item-avatar-empty"></div>';
  var timeHtml = '<span class="fwd-item-time">' + Core.formatTime(it.time || Date.now()) + '</span>';
  var inner = '';
  if (t === 'text') {
    var qHtml = '';
    if (it.quote && (it.quote.text || it.quote.stickerData || it.quote.imageData)) {
      qHtml = '<span class="msg-quote-ref">' + _quoteContentHtml(it.quote) + '</span>';
    }
    inner = '<div class="fwd-item-text">' + (qHtml || '') + _bubbleTextHtml(it.text || '') + '</div>';
  } else if (t === 'sticker' || t === 'doodle' || t === 'image') {
    var mdRef = it.stickerData || it.imageData || '';
    var srcObj = ChatMedia.imgSrcFor(mdRef);
    var cls = (t === 'image') ? 'fwd-item-img' : 'fwd-item-stk';
    var nowSrc = srcObj ? (srcObj.src || '') : '';
    if (srcObj && srcObj.ref) {
      (function(ref, key) {
        ChatMedia.getData(ref).then(function(real) {
          if (!real) return;
          var imgs = document.querySelectorAll('.fwd-media-' + idx);
          for (var k = 0; k < imgs.length; k++) { imgs[k].src = real; }
        }).catch(function() {});
      })(srcObj.ref, idx);
    }
    inner = '<img class="' + cls + ' fwd-media-' + idx + '" src="' + nowSrc + '" alt="">';
  } else if (t === 'voice') {
    inner = '<div class="fwd-item-voice">'
      + '<i class="fas fa-volume-up"></i>'
      + '<span class="voice-wave"><span></span><span></span><span></span><span></span><span></span></span>'
      + '<span class="fwd-voice-dur">' + ((it.duration || 0) + '"') + '</span></div>'
      + (it.voiceText ? '<div class="fwd-voice-text">' + Core.escapeHtml(it.voiceText) + '</div>' : '');
  } else if (t === 'redpacket') {
    inner = _renderForwardRedPacket(it);
  } else if (t === 'gift') {
    inner = _renderForwardGift(it);
  } else if (t === 'decision') {
    inner = _renderForwardDecision(it);
  } else if (t === 'forward') {
    var nestItems = it.forwardItems || [];
    inner = '<div class="fwd-nested-card">'
      + '<div class="fwd-nested-top"><i class="fas fa-history"></i> 聊天记录</div>'
      + '<div class="fwd-nested-from">来自' + Core.escapeHtml((it.forwardFrom && it.forwardFrom.name) || '对方') + '的转发</div>'
      + '<div class="fwd-nested-count">共 ' + nestItems.length + ' 条消息</div>'
      + '</div>';
  } else {
    inner = '<div class="fwd-item-text">' + Core.escapeHtml(it.text || '') + '</div>';
  }
  var tagHtml = '<span class="fwd-item-tag">' + Core.escapeHtml(_forwardItemSummaryText(it)) + '</span>';
  return '<div class="fwd-item">'
    + avatarHtml
    + '<div class="fwd-item-main">'
    + '<div class="fwd-item-head"><span class="fwd-item-name">' + nameHtml + '</span>' + timeHtml + '</div>'
    + tagHtml
    + inner
    + '</div>'
    + '</div>';
...
/* 详情层：红包（只读呈现原记录，含领取态） */
function _renderForwardRedPacket(it) {
  var amt = (it.totalAmount != null) ? it.totalAmount : (it.amount || 0);
  var statusText = it.returned ? '已退回' : (it.claimed ? '已领取' : '待领取');
  return '<div class="fwd-rp">'
    + '<div class="rp-medallion">' + GOLD_RED_PACKET_ICON_SVG + '</div>'
    + '<div class="fwd-rp-info">'
    + '<div class="fwd-rp-amount">¥' + formatRpAmountDisplay(amt) + '</div>'
    + '<div class="fwd-rp-greeting">' + Core.escapeHtml(it.greeting || '恭喜发财，大吉大利') + '</div>'
    + '<div class="fwd-rp-status">' + statusText + '</div>'
    + '</div>'
    + '</div>';
}

/* 详情层：礼物/商城卡片（只读还原） */
function _renderForwardGift(it) {
  var g = it.gift || {};
  var greeting = (g.greeting && String(g.greeting).length > 30) ? String(g.greeting).slice(0, 30) + '…' : (g.greeting || '');
  return '<div class="fwd-gift">'
    + '<div class="gift-card-head"><span class="gift-card-icon">' + (g.icon || '🎁') + '</span><span class="gift-card-name">' + Core.escapeHtml(g.name || '神秘礼物') + '</span></div>'
    + (greeting ? '<div class="gift-card-greeting">' + Core.escapeHtml(greeting) + '</div>' : '')
    + '<div class="gift-card-foot"><span class="gift-card-tag"><i class="fas fa-store"></i> 拾心商城</span><span class="gift-card-price">¥' + (g.price == null ? 0 : g.price) + '</span></div>'
    + '</div>';
}

/* 详情层：帮我抉择卡片（只读还原已答/待答现状） */
function _renderForwardDecision(it) {
  var d = it.decision || {};
  var options = d.options || [];
  var result = d.result || {};
  var answers = d.answers || [];
  var optPickNames = {};
  options.forEach(function(o, oi) { optPickNames[oi] = []; });
  answers.forEach(function(a) {
    var idx = result[a.memberId];
    if (idx !== undefined && optPickNames[idx]) optPickNames[idx].push(a.memberName || '对方');
  });
  var optHtml = '';
  options.forEach(function(o, oi) {
    var picked = (optPickNames[oi] && optPickNames[oi].length) ? ' picked' : '';
    var votes = (optPickNames[oi] && optPickNames[oi].length) ? optPickNames[oi].join('、') : '';
    optHtml += '<div class="decision-opt-item' + picked + '"><span class="opt-tag">' + (oi + 1) + '</span>'
      + '<span class="opt-text">' + Core.escapeHtml(o) + '</span>'
      + (votes ? '<span class="opt-votes">' + Core.escapeHtml(votes) + '</span>' : '')
      + '</div>';
  });
  var statusHtml = d.pending ? '抉择中…' : '抉择完成';
  return '<div class="fwd-decision decision-card">'
    ...
    + '<div class="decision-card-options">' + (optHtml || '<div class="decision-opt-empty">暂无选项</div>') + '</div>'
    + '<div class="decision-card-status' + (d.pending ? '' : ' done') + '">' + statusHtml + '</div>'
    + '</div>';
}
