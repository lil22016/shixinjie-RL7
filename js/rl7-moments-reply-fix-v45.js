/* RL7 v45 — v44 features + freeze-safe shop-card observer */
(function(){'use strict';
if(window.__RL7_V45__)return; window.__RL7_V45__=1;
...
function resolveShopCards(root){
 ...
 rows.forEach(function(row,idx){
  ...
  for(var j=idx+1;j<rows.length;j++){
   if(rows[j].classList.contains('self'))break;
   if(rows[j].classList.contains('other')){other=rows[j];break}
  }
  var txt=resultText(other);
  if(!txt){
   if(!card.classList.contains('rl7-v44-pending-check'))card.classList.add('rl7-v44-pending-check');
   return;
  }

  /* IMPORTANT: every DOM write below is idempotent.
     v44 assigned textContent on every MutationObserver pass. That assignment itself
     creates another childList mutation, which called the observer again forever and
     could lock the whole app as soon as Chat rendered. */
  if(card.classList.contains('rl7-v44-pending-check'))card.classList.remove('rl7-v44-pending-check');
  if(card.dataset.v44resolved!=='1')card.dataset.v44resolved='1';
  if(card.dataset.v43resolved!=='1')card.dataset.v43resolved='1';
  ...
  if(!st){
   ...
   card.appendChild(st);
  }else if(st.className!=='rl7-card-status rl7-v44-shop-result'){
   st.className='rl7-card-status rl7-v44-shop-result';
  }
  var wanted='Loki replied: '+txt;
  if(st.textContent!==wanted)st.textContent=wanted;
 });
}
